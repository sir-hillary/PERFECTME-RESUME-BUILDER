...
const TemplateSelector = ({selectedTemplate, onChange}) => {
    ...
    const templates = [
        ...
        },
        {
            id: "modern",
            name: "Modern",
            preview: "Sleek desing with strategic user of color and modern font choices",
        },
        {
            id: "minimal-image",
            name: "Minimal Image",
        ...
        },
        {
            id: "minimal",
            name: "Minimal",
            preview: "Ultra-clean design that puts your content fron and center",
        },
    ]
        ...
        {isOpen && (
            <div className='absolute top-full w-xs p-3 mt-2 space-y-3 z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
                {templates.map((template)=>(
                    <div key={templates.id} onClick={()=> {onChange(template.id); setIsOpen(false)}} className={`relative p-3 border rounded-md cursor-pointer transition-all ${selectedTemplate === template.id ? 
                        "border-blue-400 bg-blue-100 "  :
                        "border-gray-300 hover:border-gray-400 hover:bg-gray-100"
                    }`}>
                        {selectedTemplate === template.id && (
                            <div className='absolute top-2 right-2'>
                                <div className='size-5 bg-blue-300 rounded-full flex items-center justify-center'>
